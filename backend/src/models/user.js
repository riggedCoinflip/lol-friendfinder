...
const UserSchema = new mongoose.Schema(
  {
    name: {
      ...
      validate: {
        validator: (v) => {
          if (!userValidation.isAlphanumeric(v))
            throw new Error("Username may only contain alphanumeric chars");
          return true;
        },
      },
    },
    ...
    password: {
      ...
      validate: {
        validator: (v) => {
          if (!userValidation.containsUpper(v))
            throw new Error("Password must contain uppercase letter");
          if (!userValidation.containsLower(v))
            throw new Error("Password must contain lowercase letter");
          if (!userValidation.containsDigit(v))
            throw new Error("Password must contain a digit");
          if (!userValidation.containsOnlyAllowedCharacters(v))
            throw new Error("Password may only contain certain special chars");
    ...
    favouriteColor: {
      //temp TODO replace with better fitting field, add more fields, add fields to UserTCPublic
      type: String,
      default: "blue",
...
const UserTCPublic = composeMongoose(User, {
  ...
  onlyFields: ["name", "favouriteColor"],
});
